...
std::vector<double> > > >& fullSegResults, int flags, int kmeansIteration, double kmeansEpsilon);
...
void novelImgHandle(boost::filesystem::path testPath,boost::filesystem::path clsPath, int scale, int cropsize, int numClusters,
  int DictSize, int flags, int attempts, int kmeansIteration, double kmeansEpsilon);
